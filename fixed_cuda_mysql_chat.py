...
Question: How many records are in the resource list?
SELECT COUNT(*) FROM resource_list;

Question: How many resources are available in September 25?
SELECT COUNT(*) FROM resource_list WHERE Resource_List_availability_in_Sep_25 > 0;

Question: How many employees are in Unit-C?
SELECT COUNT(*) FROM resource_list WHERE Business_Unit = 'Unit-C';

Question: How many employees from India.
SELECT COUNT(*) FROM resource_list WHERE Country_Code = 'IND';

Question: Who are the top 5 most company experienced employees?
SELECT `Name`, `Business_Unit`, `Department`, `Company_experience_years` 
FROM rag_test.resource_list 
ORDER BY `Company_experience_years` DESC 
LIMIT 5;

Question: Who are the top 5 most industry experienced employees?
SELECT `Name`, `Business_Unit`, `Department`, `Industry_experience_years` 
FROM rag_test.resource_list 
ORDER BY `Industry_experience_years` DESC 
LIMIT 5;

Question: How many employees have experience in Oncology?
SELECT COUNT(*) FROM resource_list WHERE Therapeutic_Area LIKE '%Oncology%';

Question: Which employees in Unit-C have more than 10 years of company experience and expertise in Oncology?
SELECT `Name`, `Business_Unit`, `Department`, `Company_experience_years` FROM rag_test.resource_list WHERE Business_Unit = 'Unit-C' AND Therapeutic_Area LIKE '%Oncology%' AND `Company_experience_years` > 10;

SELECT `Name`, `Business_Unit`, `Department`, `Company_experience_years` FROM rag_test.resource_list ORDER BY `Company_experience_years` DESC LIMIT 5;
...
INSTRUCTIONS:
1. Provide a **clear and concise response** based on available data.
2. Include specific **numbers/data** in the response.
3. If **no records** are found, return `"No records found."`
4. If there's **an error**, give a simple explanation.
5. **Don't display SQL query** in the final response.
...
- **For counts:** `"There are 25 resources available."`
- **For lists:** `"I found 3 employees: Raf-Mar, Ade-Aka, and Agn-Njo."`
- **For empty results:** `"No matching records found in the database."`
- **For errors:** `"There was an issue executing the query due to a column mismatch."`

**Example Response (Based on Query Result)**

If the query result includes the following data:
[('Raf-Mar', 'Unit-B', 'Data Management', 'PDM-P', 'Principal Project Data Manager', 'GBR', 'Employee', 'Fra-son', 'English (UK),Portuguese', 'XXX0365', 'Hypercholesterolemia', 'Metabolic disease', '9.9', 'Biochemistry,Pharmacy', '20', 'Yes')]
Then the response should be as follows:
**Response:** **"Based on the query, I found 1 matching record. Raf-Mar is part of Unit-B and works in Data Management as a Principal Project Data Manager. Their expertise includes Hypercholesterolemia and Metabolic Diseases, with 9.9 years of experience in Biochemistry and Pharmacy."**

If the query result includes the following data:
[(466,)]
Then the response should be as follows:
**Response:** **"There are 466 amazing users in the database."**
